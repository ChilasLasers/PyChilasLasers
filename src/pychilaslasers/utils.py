...
class Constants:
    ...
    CRITICAL_ERRORS: list[str] = ["E0" + str(x) for x in range(14, 24)] + [
...
class CalibrationEntry:
    ...
    ]  # The prev values for but in a list
    mode_index: int | None
    cycler_index: int
...
def read_calibration_file(file_path: str | Path) -> dict:
    ...
    with open(file_path, newline="") as csvfile:
        ...
        if "[default_settings]" in csvfile.readline():
            ...
            model = (
                model.split("=")[1].strip().replace("\r", "").replace("\n", "").upper()
            )
            ...
            settings: list[str] = [
                ...
                "TUNE_TEC_TARGET",
                "TUNE_DIODE_CURRENT",
                "ANTI_HYST_PHASE_V_SQUARED",
                "ANTI_HYST_INTERVAL",
            ]

            if model != "COMET":
                settings = list(
                    filter(lambda setting: not setting.startswith("sweep"), settings)
                )
            else:
                calibration["sweep"] = {
