...
from pychilaslasers.calibration.calibration_parsing import (
...
class TestCalibration:
    ...
    def test_get_mode_hop_start(self):
        ...
        assert entry == CalibrationEntry(
            1552.0, 13.0, 22.0, 33.0, 43.0, 2, True, 3
        )  # mode hop
        assert not entry == CalibrationEntry(
            1552.0, 12.0, 23.0, 33.0, 43.0, 2, True, 4
        )  # mode hop
        assert not entry == CalibrationEntry(
            1552.0, 11.0, 24.0, 33.0, 43.0, 2, False, 5
        )  # mode hop
